            ...
            {isShowButtonLeftRight &&
                <>
                    <div
                        ...
                        className="absolute z-20 hidden text-xl transition-transform -translate-y-1/2 cursor-pointer md:text-2xl md:text-4xl group-hover/navigate:block top-2/4 left-4"
                    >
                        <FontAwesomeIcon color="white" className="opacity-50 hover:opacity-100" icon={faChevronLeft} />
                    </div>
                    <div
                        onClick={goToNext}
                        className="absolute z-20 hidden text-xl transition-transform -translate-y-1/2 cursor-pointer md:text-2xl md:text-4xl group-hover/navigate:block top-2/4 right-4"
                    >
            ...
            {slides.map((slide, index) => (
                <div key={index} className={`transition-all duration-500 ease-in-out ${index === currentIndex ? ' opacity-100' : ' opacity-10'}`} >
                    <img
                        src={slide.url}
                        alt={`Slide ${index + 1}`}
                        className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-500 ease-linear z-10 ${index === currentIndex ? 'opacity-100' : 'opacity-0'}`}
                    />
                    <div className={`flex absolute w-full md:w-2/3 lg:w-1/2 pr-10 lg:p-8 flex-col md:gap-2 lg:gap-5 transition-all duration-500 delay-700 left-6 md:left-12 lg:left-20 bottom-[6%] ease-in-out z-10 ${index === currentIndex ? 'opacity-100' : 'opacity-0'} `}>
                        ...
                        <div className={`delay-[1000ms] hidden md:block transition-all duration-500 ease-in-out text-gray-300 text-sm font-semibold ${index === currentIndex ? '-translate-y-8 opacity-100' : 'opacity-0'}`}>
                            {slide.description}
                        </div>
